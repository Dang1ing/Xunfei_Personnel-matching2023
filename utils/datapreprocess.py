"""
Preprocess Criteo dataset. This dataset was used for the Personnel Matching
Challenge (https://challenge.xfyun.cn/topic/info?type=person-post-matching-2023).
"""
import os
import sys
import click
    ...
    with open(os.path.join(outdir, 'train.txt'), 'w') as out_train:
        with open(os.path.join(datadir, 'train.txt'), 'r') as f:
            for line in f:
                ...
                label = features[0]
                out_train.write(
                    ','.join([continous_vals, categorial_vals, label]) + '\n')
        ...
        with open(os.path.join(datadir, 'test.txt'), 'r') as f:
            for line in f:
                ...
                for i in range(0, len(categorial_features)):
                    ...
                    val = dicts.gen(
                        i, features[categorial_features[i] - 1])  # 修改过
                    categorial_vals.append(str(val))
